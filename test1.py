...
def __main__():
  ...
  ### should i remove the initial conditions
    
    ###Test0: high accuracy solutions, these results are interesting, however take too long to compute###
#    nx = 401
#    nt = 1001
#    x = np.linspace(0,1,nx) 
...
#    plt.show()
    ...
    ### Test2: Problems and limitations with ftcs method
    
        ### efficient code, no matrix solved
        ### restrictions on the ftcs methodology
        ### small spatial discretisation deals with shock badly
        ### the timestep conditions:
    ...
    x = np.linspace(0,1,nx) 
    plt.figure(figsize= (9,5)) 
    EA = SolveBurger(nx,nt,1,0.001,"picard",0,"conservative","jump")
    plt.plot(x, EA, color = 'r' ,linewidth=0.5 ,label = "mu = 0.001, nx = %a ,nt = %a " %(nx,nt))
    plt.ylabel('$\phi$')
    plt.xlabel('x')
    plt.legend()
    plt.title("ftcs-conservative when it's effective  mu approx nx")
    plt.show()  
    print("nx is big mu small, we have the timestepping restriction being unusually weak")
    ...
    plt.xlabel('x')
    plt.legend()
    plt.title("ftcs-conservative when it's ineffective")
    plt.show()  
    ...
    plt.xlabel('x')
    plt.legend()
    plt.title("ftcs-conservative when it's ineffective: not resolving the jump")
    plt.show()  
    print("nx needs to be large, to deal with the shock, and as mu tends to zero \
          ...
          hence the method becomes absolutely unuseable")   
...
### Test3: forward time centered difference method as spatial discretisation increases ###
    ...
    for j in range(0,4):
        ...
        plt.title("ftcs")
    plt.show()
    #b) for conservetive form set "non conservative" to "conservative"
    ntf = np.zeros([4])
    ...
    ###----- Test: nx = 101 comparisons of different methods as mu varies -----####
    ...
    plt.plot(x, X, color = 'g' ,linewidth=0.5,label = "newton non conservative" )
    X = SolveBurger(nx,nt,1,0.001,"picard",0,"conservative","jump")
    plt.plot(x, X, color = 'b' ,linewidth=0.5,label = "newton conservative" )
    X = SolveBurger(nx,nt,1,0.001,"picard",0,"non conservative","jump")
    plt.plot(x, X, color = 'r' ,linewidth=0.5,label = "newton non conservative" )
    plt.ylabel('$\phi$')
    plt.xlabel('x')
    plt.legend()
    plt.title("nx =101, nt = 501")
